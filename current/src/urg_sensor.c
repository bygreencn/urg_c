                         ...
                         char *receive_buffer, int receive_buffer_max_size)
{
    ...
    //printf("****%s (%d) %d\n",command,n,receive_buffer_max_size);
    if (n != write_size) {
    ...
    do {
	    ...
	    // \~english Checks the checksum value for all lines except the echoback (in SCIP1.1 this is ignored)
            if(urg->is_urg_04lx && !strncmp(command,"VV",2))
            {
                ;
                //printf("URG_04LX VV don't check checksum\n");
            } else {             
...
{
    long try_baudrate[] = { 19200, 38400, 57600, 115200, 250000, 500000, 750000 };
...
static int receive_parameter(urg_t *urg)
{
    ...
    for (i = 0; i < (ret - 1); ++i) {
        if (!strncmp(p, "MODL:", 5)) {
            if(!strncmp(p+5,"URG-04LX",8))
                urg->is_urg_04lx = URG_TRUE;
            else
                urg->is_urg_04lx = URG_FALSE;
             ...
             const char *device_or_address, long baudrate_or_port)
{
    ...
    urg->is_urg_04lx = URG_FALSE;
    urg->is_urg_04lx_init = URG_FALSE;
    memset(urg->urg_04lx_vvbuffer,0,sizeof(urg->urg_04lx_vvbuffer));
...
static char *copy_token(char *dest, const char *receive_buffer,
...
const char *receive_version_info(urg_t *urg)
{
    enum {
    ...
    if(urg->is_urg_04lx && urg->is_urg_04lx_init)
    {
        //dump_buffer(urg->urg_04lx_vvbuffer,0);
        return (const char *)(urg->urg_04lx_vvbuffer);
    } else {
        ...
        memcpy(urg->urg_04lx_vvbuffer,receive_buffer,sizeof(receive_buffer));
        if(urg->is_urg_04lx)
            urg->is_urg_04lx_init = URG_TRUE;
        //dump_buffer(urg->urg_04lx_vvbuffer,0);
        return (const char *)(urg->urg_04lx_vvbuffer);
    } 
}
...
const char *urg_sensor_product_type(urg_t *urg)
{
    ...
    ret = receive_version_info(urg);
    if (!ret) {
        return ret;
    }

    p = copy_token(urg->return_buffer,
	               ret, "PROD:", ";", (urg->is_urg_04lx)? (VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
...
const char *urg_sensor_serial_id(urg_t *urg)
{
    ...
    p = copy_token(urg->return_buffer,
                   ret, "SERI:", ";", (urg->is_urg_04lx)?(VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
...
const char *urg_sensor_vendor(urg_t *urg)
{
    ...
    p = copy_token(urg->return_buffer,
                   ret, "VEND:", ";", (urg->is_urg_04lx)? (VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
    return (p) ? p : RECEIVE_ERROR_MESSAGE;
}
...
const char *urg_sensor_firmware_version(urg_t *urg)
{
    ...
    p = copy_token(urg->return_buffer,ret, "FIRM:", "(", (urg->is_urg_04lx)? (VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
...
const char *urg_sensor_firmware_date(urg_t *urg)
{
    ...
    char *date;
    p = copy_token(urg->return_buffer,
               ret, "FIRM:", ";", (urg->is_urg_04lx)? (VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
...
const char *urg_sensor_protocol_version(urg_t *urg)
{
    ...
    p = copy_token(urg->return_buffer,
                   ret, "PROT:", ";", (urg->is_urg_04lx)? (VV_RESPONSE_LINES+1):VV_RESPONSE_LINES);
    return (p) ? p : RECEIVE_ERROR_MESSAGE;
}
